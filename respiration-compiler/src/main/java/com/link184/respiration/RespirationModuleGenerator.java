    ...
    private List<MethodSpec> generateGetters(Map.Entry<TypeElement, String> entry) {
        ...
        for (Element element : enclosedElements) {
            if (element.getAnnotation(RespirationRepository.class) != null) {
                getters.add(generateGetter(element));
            }
        }
        return getters;
    }

    private MethodSpec generateGetter(Element element) {
        ...
        ParameterizedTypeName returnType = ParameterizedTypeName.get(ClassName.bestGuess(element.asType().toString()), modelType);
        return MethodSpec.methodBuilder("get" + capitalizedRepoName)
                .addModifiers(Modifier.PUBLIC)
                .returns(returnType)
                .addStatement("return $N.$N()", capitalizedRepoName + RepositoryBuilderGenerator.CLASS_NAME_SUFFIX,
